...
AQRESS Pulse is a configurable IoT sensor and device management platform. This repository contains the **V0.1.1 Phase 7 Sensor Telemetry Ingestion** application, built on the foundations delivered in Phases 1–6.

Devices authenticate to EMQX with individually provisioned credentials and broker-enforced per-Device topic ACLs. The MQTT control worker tracks heartbeat and configuration acknowledgments; a separate telemetry worker validates, resolves, and persists typed Sensor Readings generated by the optional simulator. Dashboards, charts, live browser updates, deployment, and cloud infrastructure are intentionally not implemented yet.
...
EMQX delegates password authentication to the internal-only `mqtt-auth` service. Its successful response contains an exact per-Device ACL, and the broker applies default-deny authorization. A Device may publish only its own heartbeat, configuration acknowledgment, and telemetry topic, and may subscribe only to its own configuration and reserved command topic. The separate platform identity publishes Device configurations and subscribes to control topics and the wildcard telemetry topic. `mqtt-auth` has no host port and does not expose API documentation.
...
aqress/pulse/v1/devices/{device_uid}/telemetry   # consumed by telemetry-worker
...
Configuration snapshots use QoS 1 and retained delivery. The control worker subscribes only to `status` and `config/ack`; the separate telemetry worker subscribes only to `+/telemetry`. Valid heartbeats set the Device `ONLINE` and update `last_seen` with server receive time. An `ONLINE` Device becomes `OFFLINE` after `DEVICE_OFFLINE_TIMEOUT_SECONDS` without a heartbeat. Telemetry deliberately does not modify connectivity state.
...
The simulator publishes heartbeats, receives retained configuration snapshots, keeps supported configuration in memory, publishes acknowledgments, and generates telemetry only for successfully applied, enabled Sensors and Channels. Set `SIMULATOR_FORCE_CONFIG_FAILURE=true` to test a deterministic `FAILED` acknowledgment. Stop it and wait for the configured timeout to verify the `OFFLINE` transition.
...
The optional simulator batches due measurements into the Device-owned telemetry topic. Telemetry uses QoS 1 with `retain=false`; configuration remains retained. `SIMULATOR_RANDOM_SEED` makes development values repeatable. The supported generators are:

- `ds18b20`: gradually varying bounded temperature in °C.
- `bme280`: independently bounded temperature, humidity, and pressure.
- `digital_input`: deterministic boolean state transitions.
- `analog_input`: gradual numeric values within `engineering_min`/`engineering_max`, or the documented development range `0–100` when absent.

The telemetry payload is:

```json
{
  "message_id": "018f25d0-9d37-7bc6-92ab-7b2ad0fa32e1",
  "device_uid": "ESP32-A8C339",
  ...
  "readings": [
    {
      "sensor_uid": "ENV-001",
      "channel": "temperature",
      "recorded_at": "2026-08-22T15:59:59Z",
      "value": 24.7,
      "quality": "GOOD"
    }
  ]
...
The envelope must contain a UUID `message_id`, matching topic/payload Device UID, timezone-aware `sent_at`, and between 1 and `TELEMETRY_MAX_READINGS_PER_MESSAGE` readings (default `100`). An invalid envelope rejects the whole MQTT message. Once the envelope is valid, each reading is resolved through `device_uid → (device_id, sensor_uid) → Measurement Definition key`. Unknown/disabled/error Sensors, unknown/disabled Channels, type mismatches, invalid timestamps, and readings beyond `TELEMETRY_MAX_FUTURE_SKEW_SECONDS` are rejected individually while other valid readings in the same batch continue.

NUMERIC, TEXT, and BOOLEAN values are stored in separate typed PostgreSQL columns with a CHECK constraint requiring exactly one. JSON booleans are never accepted as numbers, and strings/numbers are not coerced. `GOOD`, `UNCERTAIN`, and `BAD` are persisted quality values; correctly structured `BAD` measurements remain valid data.

`recorded_at` is the Device measurement time. `received_at` is generated by AQRESS Pulse at ingestion. Both are UTC `TIMESTAMPTZ`. The current Sensor Channel unit is copied into every Reading as a historical unit snapshot, and each individual reading object is retained in `raw_payload` JSONB.

QoS 1 duplicate delivery is idempotent through `UNIQUE(device_id, message_id, reading_index)`, where `reading_index` is the zero-based position in the published batch. Valid rows are inserted together in one transaction with conflict-ignore semantics; no reading mutation or delete API exists.

Authenticated read-only endpoints are:

- `GET /api/v1/sensors/{sensor_id}/latest` — one newest Reading per enabled Channel, selected in PostgreSQL by `recorded_at DESC, id DESC`.
- `GET /api/v1/sensors/{sensor_id}/readings` — bounded Sensor history.
- `GET /api/v1/sensors/{sensor_id}/channels/{channel_id}/readings` — bounded Channel history after ownership validation.

History accepts timezone-aware `from`, `to`, `limit`, and opaque `cursor` parameters. The default limit is `100`, maximum `1000`, and stable keyset ordering is `recorded_at DESC, id DESC`. Responses contain `items`, `next_cursor`, and `has_more`; values are exposed through one correctly typed `value` field.

Example API verification after signing in and setting `TOKEN`:

```bash
curl --fail --header "Authorization: Bearer $TOKEN" \
  http://localhost:8000/api/v1/sensors/SENSOR_UUID/latest

curl --fail --get --header "Authorization: Bearer $TOKEN" \
  --data-urlencode 'limit=100' \
  --data-urlencode 'from=2026-08-22T00:00:00Z' \
  http://localhost:8000/api/v1/sensors/SENSOR_UUID/readings
```

The current indexes support newest-first Device, Sensor, Channel, and global time queries. Native PostgreSQL time partitioning, aggregation/downsampling, and retention policies are deferred until telemetry volume justifies them.
...
simulator/               Phase 7 authenticated Device telemetry simulator and tests
...
The MQTT control worker lives under `backend/app/mqtt`; the independent telemetry data-plane worker and validation pipeline live under `backend/app/telemetry`.
...
- PostgreSQL contains the Phase 1–6 tables plus append-only `sensor_readings`.
...
Phase 7 implements deterministic Sensor telemetry generation, broker-isolated QoS 1 delivery, a separate ingestion worker, typed/idempotent Reading persistence, and authenticated latest/history APIs. It does **not** implement charts, dashboards, live browser updates, alerts, aggregation/downsampling, retention policies, analytics, AI, equipment commands, deployment, or any Phase 8+ work.
