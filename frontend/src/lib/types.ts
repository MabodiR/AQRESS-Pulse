...
export type ConfigurationStatus = 'PENDING' | 'PUBLISHED' | 'APPLIED' | 'FAILED' | 'SUPERSEDED'
export type SensorChannel = { id: string; sensor_id: string; measurement_definition_id: string; key: string; name: string; unit: string | null; enabled: boolean; created_at: string; updated_at: string }
export type SensorConfiguration = { id: string; sensor_id: string; config_version: number; configuration: Record<string, unknown>; status: ConfigurationStatus; is_current: boolean; created_at: string; updated_at: string; published_at: string | null; applied_at: string | null }
export type Sensor = { id: string; device_id: string; sensor_type_id: string; sensor_uid: string; name: string; description: string | null; status: SensorStatus; enabled: boolean; created_at: string; updated_at: string; device: Pick<Device, 'id' | 'device_uid' | 'name' | 'site_id'> & { site: Pick<Site, 'id' | 'name'> }; sensor_type: Pick<SensorType, 'id' | 'code' | 'name' | 'interface_type'>; channels: SensorChannel[]; current_configuration: SensorConfiguration }
