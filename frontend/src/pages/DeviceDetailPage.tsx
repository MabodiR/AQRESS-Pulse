...
export function DeviceDetailPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () => api<Device>(`/devices/${deviceId}`),
    refetchInterval: 5000,
  });
  const sensors = useQuery({
    queryKey: ["device-sensors", deviceId],
    queryFn: () =>
      api<Paginated<Sensor>>(`/devices/${deviceId}/sensors?page_size=100`),
    refetchInterval: 5000,
  });
  const status = useMutation({
    mutationFn: (active: boolean) =>
      api<Device>(`/devices/${deviceId}/status`, {
        method: "PATCH",
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          user?.role !== "VIEWER" ? (
            <Link className="button-primary" to={`/devices/${device.id}/edit`}>
              Edit Device
            </Link>
          ) : undefined
        }
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        {user?.role !== "VIEWER" ? (
          <button
            ...
            onClick={() => status.mutate(!device.is_active)}
          >
            {device.is_active ? "Disable device" : "Enable device"}
          </button>
        ) : null}
      </div>
      <dl className="panel grid gap-x-8 gap-y-6 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <DeviceConnectivityPanel device={device} role={user?.role} />
      <section className="panel mt-7">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-bold">Sensors</h2>
            <p className="mt-1 text-sm text-slate-400">
              Physical Sensors attached to this Device.
            </p>
          </div>
          {user?.role !== "VIEWER" && device.is_active ? (
            <Link
              className="button-primary"
              to={`/devices/${device.id}/sensors/new`}
            >
              + Add Sensor
            </Link>
          ) : null}
        ...
        {sensors.error ? (
          <div className="mt-5">
            <ErrorNotice error={sensors.error} />
          </div>
        ) : null}
        {sensors.isPending ? (
          ...
          <div
            ...
            tabIndex={0}
            aria-label="Device Sensors table"
          >
            <table>
              <thead>
                <tr>
              ...
              <tbody>
                {sensors.data.items.map((sensor) => (
                  <tr key={sensor.id}>
                    ...
                    <td>{sensor.channels.length}</td>
                    <td>
                      <StatusBadge status={sensor.status} />
                    </td>
                    <td>
                      <StatusBadge
                        status={sensor.current_configuration.status}
                      />
                    </td>
                  </tr>
                ))}
          ...
          <div className="mt-5">
            <Empty>No Sensors are attached to this Device yet.</Empty>
          </div>
        )}
      </section>
    </>
